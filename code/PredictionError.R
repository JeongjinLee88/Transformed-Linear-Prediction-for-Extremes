#' Prediction error via tail ratio
#' @description
#' Construct a regularly varying random vector \eqn{\boldsymbol{X}=A\circ \boldsymbol{Z}}
#' from a \eqn{p \times q} matrix \eqn{A} applied to a random vector \eqn{\boldsymbol{Z}} 
#' of unit-scale Pareto random variables with \eqn{\alpha=2}.
#' The function generates random realizations of \eqn{\boldsymbol{X}} and returns true TPDM
#' , true prediction TPDM, the optimized vector b, and \eqn{\hat{X}}.
#' 
#' @param Ncol Numeric; the column dimension of a matrix A
#' @param Nrow Numeric; the row dimension of a matrix A
#' @param n Numeric; sample size
#' @param min Numeric; the lower bound of a uniform distribution
#' @param max Numeric; the upper bound of a uniform distribution
#' @param plot_D Logical; return the scatterplot of D versus Xhat (default)
#'
#' @return A list of numeric results: 
#' 1. true prediction TPDM from a specified matrix A
#' 2. the optimized vector b
#' 3. a vector of D, 'D_TPDM'
#' 4. assessed coverage rate, 'Coverage_D'
#' @export
#'
#' @examples 
